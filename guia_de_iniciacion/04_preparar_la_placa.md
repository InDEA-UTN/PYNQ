...
```text
       ...
       |  J9 (Power Source) --> Seleccionar "USB/REG"|
       +-----------------------------------------------+
```
...
### 2. Fuente de Alimentación (`J9` - Power Selection)
* **Opción A (USB):** Si se alimenta la placa mediante el puerto Micro-USB (PROG/UART), colocar `J9` en la posición **USB**.
* **Opción B (REG - Recomendado):** Si se utiliza una fuente de alimentación externa de 5V DC mediante el conector jack, colocar `J9` en la posición **REG** (Regulador interno).
---
...
2. **Conectar el cable Micro-USB:**
   * Conectar el cable Micro-USB al puerto rotulado como **PROG / UART** de la placa y a un puerto USB de la PC (esto proveerá comunicación serie por consola y alimentación si `J9` está en USB).
3. **Encender la placa:**
   * Deslizar el interruptor de encendido **SW_3PIN** a la posición **ON**.
<details>
  <summary>📸 Ver imagen del switch SW_3PIN</summary>
  <br>
  <img src="./imagenes/SW_3PIN.png" alt="Ubicación del switch PSON_SW" width="400">
</details>
---
...
1. **Power LED (`CR8` / Red):** Se enciende inmediatamente en rojo continuo al mover el switch a la posición ON, indicando que la placa recibe energía.
2. **DONE LED (`CR7` / Green):** Después de unos segundos (~5-10 s), el LED **DONE** se ilumina en verde. Esto confirma que la lógica reconfigurable (FPGA) cargó exitosamente el *bitstream* base.
3. **LEDs de actividad del procesador (`LD0`-`LD3` / Ethernet):** Los LEDs de usuario inmediatamente arriba de `BTN3` - `BTN0` parpadearán indicando el arranque del kernel de Linux Ubuntu y la inicialización de los servicios de Jupyter Notebook (el proceso completo toma alrededor de 30 a 45 segundos).
...
* **La placa se reinicia intermitentemente:**
  * Si se alimenta por USB, la PC o puerto USB puede no estar entregando la corriente suficiente (mínimo 2A-3A). Cambiar a un cargador de pared dedicado o usar una fuente jack DC de 5V (jumper `J9` en **REG**).
